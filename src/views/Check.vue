  ...
  <div class="pos">
    <div>
      <el-row>
          ...
          <el-tabs stretch>
            <el-tab-pane label="点餐">
              ...
              <div class="order-footer">
                <div class="totalDiv">
                  <small>数量：</small>
                  <strong style="color:rgb(252,0,5)">{{totalCount}}</strong> &nbsp;&nbsp;&nbsp;&nbsp;
                  <small>总计：</small>
                  <strong style="color:rgb(252,0,5)">{{totalMoney}}</strong> 元
                </div>
        ...
        <!--商品展示-->
        <el-col :span="16" class="goods-show">
              ...
              <el-tab-pane label="汉堡">
                <ul class="cookList">
                  <li v-for="goods in type0Goods" @click="addOrderList(goods)" :key="goods.goodsId">
                    <span class="foodImg">
                      <img :src="goods.goodsImg" :onerror="errorImg" width="100%" />
                    </span>
...
import oftengoods from "@/json/oftenGoods.json";
import typegoods from "@/json/typeGoods.json";
export default {
  name: "Pos",
  mounted: function() {
  ...
  created() {
    ...
    this.type0Goods = typegoods.type0Goods;
    this.type1Goods = typegoods.type1Goods;
    this.type2Goods = typegoods.type2Goods;
    this.type3Goods = typegoods.type3Goods;
  },
  data() {
    return {
      tableData: [], //订单列表的值
  ...
  methods: {
    ...
    deleteOne(data) {
      ...
      if (arr.length > 0) {
        arr[0].count--;
        if (arr[0].count == 0) {
          this.tableData.splice(this.tableData.indexOf(arr[0]), 1);
        }
      }
      this.getAllMoney();
    },
